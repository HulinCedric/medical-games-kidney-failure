 ...
 * @author Cedric Hulin
 */
public class EcouteurPyramideClavier implements MouseListener, MouseMotionListener {
	 ...
	 * @since 1.0.0
	 */
	public EcouteurPyramideClavier(Pyramide hamecon) {
	...
	public void mouseClicked(MouseEvent e) {
		...
		//
		hamecon.obtenirClavier().modifierCouleurEcran(Color.white);
		...
		if (((PanneauG) e.getComponent()).presenceImage()) {
			...
			//
			hamecon.modifierCheminImage(((PanneauG) e.getComponent()).obtenirCheminImage());

			// Afficher le nom de l'aliment selectionne dans
			// le clavier
			//
			File fichier = new File(hamecon.obtenirCheminImage());
			hamecon.obtenirClavier().modifierTexteEcran(fichier.getName().substring(0, fichier.getName().length() - 4));

			// Specifier l'element selectionne
			//
			hamecon.obtenirClavier().modifierCouleurElement(new Color(0, 0, 0, 0));
			((PanneauG) e.getComponent()).setBackground(Color.green);
			hamecon.obtenirClavier().repaint();
		}
	}
